...
const assets=['cinematic.css','cinematic.js','silk-heritage.css','silk-heritage.js',...['love-rose','love-pearl','wedding-silk','wedding-garden','birthday-aurora','apology-rain','proposal-pearl'].map(n=>n+'.html')];
await Promise.all(assets.map(file=>fs.access(path.join(ROOT,'templates',file))));
await fs.access(path.join(ROOT,'assets','silk-heritage-original.jpg'));
for(const name of ['cinematic.js','silk-heritage.js']){const jsCheck=spawnSync(process.execPath,['--check',path.join(ROOT,'templates',name)],{encoding:'utf8'});if(jsCheck.status!==0)throw new Error('EMORA '+name+' syntax check failed: '+(jsCheck.stderr||'unknown'))}
